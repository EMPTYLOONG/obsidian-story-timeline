...
一个为 Obsidian 打造的横向时间线视图。支持多轨道分类、时期事件、子事件分割、标签筛选、时间游标、可视化编辑器、自定义代表色和图片导出。
...
- **子事件对比**：子事件块使用更深的半透明色，与父条带背景形成层次区分
...
- **导出图片**：一键将整条时间线导出为 PNG 截图（自动适配超长尺寸）
...
支持**新增**（带二次确认）、**删除**（带二次确认）、**检查重复**（显示名冲突会高亮警告）。
...
用**事件编辑器**设置颜色会自动加引号，不会出问题。如果手动编辑 frontmatter，记得加引号。

### 分类名称有长度限制

显示名称最长 **20 字符**，超过部分会被自动截断。这是为了避免：

- 侧边栏里过长的分类名挤出删除按钮
- 时间线轨道标签过长

如果 20 字符不够用，可以修改 `src/settings.ts` 里的常量：
